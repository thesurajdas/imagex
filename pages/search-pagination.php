...
if(!empty($_GET['s'])){
    $s = $_GET['s'];
    $sql="SELECT * FROM images WHERE visibility=0 AND title LIKE '%{$q}%' ORDER BY {$s} DESC LIMIT {$page},$limit";
}else{
                                    ...
                                    <div class="col-lg-4 col-md-6 col-sm-12 sglry">
                                        <div class="card cds">
                                            ...
                                            <div class="card-text cds-txt">
                                                ...
                                                echo $site_url.'/pages/profile.php?u='.$username; ?>" class=" text-decoration-none text-white"><img draggable="false" class="upimg" src="<?php echo $site_url."/".$user_avatar; ?>" alt="" onContextMenu="return false;" style="object-fit: cover;"> <?php echo $fullname; ?></a>
                                                <div class="container">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-2 text-center pr-0 chbtn">
                                                            <button  data-toggle="modal" data-target="#shareimg" onclick="shareimgpop(<?php echo $row['id'];?>);" type="button" class="btn  pr-0" style="color:white"><i class="fad fa-share-square"></i></button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
